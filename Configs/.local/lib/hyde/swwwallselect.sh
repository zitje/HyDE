...
rofiSel=$(paste <(printf "%s\n" "${wallListBase[@]}") <(printf "%s\n" "${wallHash[@]}") | awk -v thmbDir="${thmbDir}" '{split($1, arr, "/"); print arr[length(arr)] "\x00icon\x1f" thmbDir "/" $2 ".sqre"}' | rofi -dmenu -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofiConf}" -select "${currentWall}")
