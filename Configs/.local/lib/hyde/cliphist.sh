...
if [ $# -eq 0 ]; then
    main_action=$(echo -e "History\nDelete\nView Favorites\nManage Favorites\nClear History" | rofi -dmenu -theme-str "entry { placeholder: \"Choose action\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
else
    main_action="History"
fi
...
case "${main_action}" in
"History")
    selected_item=$(cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"History...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
    ...
    ;;
"Delete")
    selected_item=$(cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"Delete...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
    if [ -n "$selected_item" ]; then
...
"View Favorites")
    if [ -f "$favoritesFile" ] && [ -s "$favoritesFile" ]; then
        ...
        selected_favorite=$(printf "%s\n" "${decoded_lines[@]}" | rofi -dmenu -theme-str "entry { placeholder: \"View Favorites\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
    ...
    ;;
"Manage Favorites")
    manage_action=$(echo -e "Add to Favorites\nDelete from Favorites\nClear All Favorites" | rofi -dmenu -theme-str "entry { placeholder: \"Manage Favorites\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")

    case "${manage_action}" in
    "Add to Favorites")
        # Show clipboard history to add to favorites
        item=$(cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"Add to Favorites...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
        if [ -n "$item" ]; then
    ...
    "Delete from Favorites")
        if [ -f "$favoritesFile" ] && [ -s "$favoritesFile" ]; then
            ...
            selected_favorite=$(printf "%s\n" "${decoded_lines[@]}" | rofi -dmenu -theme-str "entry { placeholder: \"Remove from Favorites...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
            if [ -n "$selected_favorite" ]; then
                index=$(printf "%s\n" "${decoded_lines[@]}" | grep -nxF "$selected_favorite" | cut -d: -f1)
                if [ -n "$index" ]; then
                    selected_encoded_favorite="${favorites[$((index - 1))]}"
    ...
    "Clear All Favorites")
        if [ -f "$favoritesFile" ] && [ -s "$favoritesFile" ]; then
            confirm=$(echo -e "Yes\nNo" | rofi -dmenu -theme-str "entry { placeholder: \"Clear All Favorites?\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")
            if [ "$confirm" = "Yes" ]; then
                : >"$favoritesFile"
    ...
    *)
    ...
    ;;
"Clear History")
    if [ "$(echo -e "Yes\nNo" | rofi -dmenu -theme-str "entry { placeholder: \"Clear Clipboard History?\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${rofi_config}")" == "Yes" ]; then
        cliphist wipe
